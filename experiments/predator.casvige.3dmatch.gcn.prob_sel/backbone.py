...
from vision3d.ops import knn_interpolate_pack_mode, nearest_interpolate_pack_mode


class PointEncoder(nn.Module):
    def __init__(self, input_dim, init_dim, kernel_size, init_radius, init_sigma):
        super().__init__()
...
class PointDecoder(nn.Module):
    def __init__(self, input_dim, output_dim, init_dim):
        super().__init__()

        self.decoder3 = UnaryBlockPackMode(input_dim + init_dim * 8, init_dim * 8)
    ...
    def forward(self, feats_list, data_dict):
        points_list = data_dict["points"]
        upsampling_list = data_dict["upsampling"]

        feats_s1, feats_s2, feats_s3, feats_s4 = feats_list
